...
- **Font**: Inter (variable) for body copy; condensed display stack for high-impact headings; `0.875rem` body
- **Primary**: perceptual OKLCH emerald (`oklch(62% 0.16 151)`) in light mode and a lighter emerald in dark mode
- **Surfaces**: warm paper neutrals in light mode; deep navy in dark mode
- **Geometry**: square, ruled inspection-bench surfaces with a `0.3rem` base radius reserved for small controls
- **Depth**: borders and tonal hierarchy first; ambient shadows only for genuinely elevated surfaces
- **Pattern**: restrained 48px page grid and 24px file-workspace grid
- **Icons**: Lucide React, 16–24px range, usually inside square outlined markers
