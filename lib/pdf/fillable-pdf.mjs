...
export async function createFillablePdf(input, fields) {
  ...
  // Field coordinates are canonical, unrotated PDF page coordinates. The
  // preview owns viewport conversion; doing another conversion here drifts
  // fields on rotated pages.
  ...
  for (let i = 0; i < fields.length; i += 1) {
    ...
    const rawWidth = Math.abs(Number(field.width) || 0) || defaults.width;
    const rawHeight = Math.abs(Number(field.height) || 0) || defaults.height;
