...
JPG uses lossy compression. Lowering quality can reduce file size, but the visible difference depends on the image and encoder. Compare fine detail and gradients before choosing a setting.

PNG can store pixels losslessly, but this tool's PNG compression can reduce the number of colors before encoding. That quantization changes pixel values and may introduce banding or affect small text. Keep the original when exact pixel preservation matters.
...
For PNG output, inspect gradients, transparency edges, and small text for changes from color reduction. A PNG filename alone does not guarantee that an optimization preserved every original pixel.
