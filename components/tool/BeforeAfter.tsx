...
export default function BeforeAfter({ originalSize, compressedSize, filename }: BeforeAfterProps) {
  ...
  return (
    <div className="animate-slide-up flex items-center gap-3 rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm">
      ...
      <span className="font-medium tabular-nums">{formatBytes(compressedSize)}</span>
      {isSmaller ? (
        <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/30 dark:text-green-400 tabular-nums">
